...
const PawAssistButton = ({ onClick, isActive = false, disabled = false }) => {
  const { theme } = useTheme();
  const [isHovered, setIsHovered] = useState(false);

  return (
    <StyledWrapper
      className={`paw-assist-button ${isActive ? 'active' : ''} ${disabled ? 'disabled' : ''}`}
      theme={theme}
      onClick={disabled ? undefined : onClick}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      title="Paw Assist - AI Scripting Assistant"
    >
      <div className="button-content">
        ...
        <span className="button-text">Paw Assist</span>
      </div>
    </StyledWrapper>
  );
};

export default PawAssistButton;
