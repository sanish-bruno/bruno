...
describe('refreshShellEnvProxyVars — shell inheritance (POSIX)', () => {
  ...
  test('deletes stale process.env proxy vars BEFORE invoking the shell subprocess', async () => {
    process.env.http_proxy = 'http://stale:8080';
    process.env.HTTPS_PROXY = 'http://stale:8443';
    mockShellEnvResult = { http_proxy: 'http://new:8080' };

    await refreshShellEnvProxyVars();

    // The shell subprocess must have been invoked with a clean slate — that is the whole point
    // of the delete-first design (so removed .zshrc exports don't inherit from the parent env).
  ...
  test('cleans stale uppercase var when shell only exports lowercase equivalent', async () => {
    process.env.HTTP_PROXY = 'http://old-uppercase:8080';
    mockShellEnvResult = { http_proxy: 'http://new-lowercase:8080' };

    await refreshShellEnvProxyVars();

    expect(process.env.HTTP_PROXY).toBeUndefined();
    expect(process.env.http_proxy).toBe('http://new-lowercase:8080');
  });
