  ...
  try {
    ...
    const transformedJson = {
      ...
      response: {
        ...
        status: _.get(json, 'response.status.code', 200),
    ...
    console.log('bruExampleToJson transformedJson', transformedJson);
...
export const jsonExampleToBru = (json: any, parentType?: string, parentMethod?: string) => {
  console.log('jsonExampleToBru json', json);
  try {
    const requestMethod = parentMethod || _.get(json, 'request.method', 'GET');
